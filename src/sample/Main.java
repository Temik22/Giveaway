...
public class Main extends Application {
    ...
    private Parent paint() {
        ...
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) {
                ...
                if (y <= 2 && (x + y) % 2 != 0) {
                    piece = makePiece(x, y, Piece.RED, false);
                }

                if (y >= 5 && (x + y) % 2 != 0) {
                    piece = makePiece(x, y, Piece.BLUE, false);
                }
    ...
    private Piece makePiece(int x, int y, int team, boolean isKing) {
        Piece piece = new Piece(x, y, team, isKing);
    ...
    private boolean checkForNext(int newX, int newY, int team, boolean isKing) {
        if (team != Piece.BLUE || isKing) {
        ...
        for (int i = 0; i < WIDTH; i++) {
            for (int j = 0; j < HEIGHT; j++) {
                if (gameData[i][j].havePiece() && gameData[i][j].getPiece().getTeam() == team)
                    if (checkForNext(i, j, gameData[i][j].getPiece().getTeam(), gameData[i][j].getPiece().isKing))
                        return true;
            }
        }
        return false;
    ...
    private MoveResult tryMove(Piece piece, int newX, int newY) {
        ...
        if (NeedToEat != null && NeedToEat != piece) {
            return new MoveResult(MoveType.NONE);
        }
        ...
        if (piece.isKing) {
            ...
            if (Math.abs(newX - x0) == 2 && Math.abs(newY - y0) == 2) {
                ...
                if (gameData[x1][y1].havePiece() && gameData[x1][y1].getPiece().getTeam() != piece.getTeam()) {
                    if (!checkForNext(newX, newY, piece.getTeam(), piece.isKing))
                        setTurn(piece.getTeam());
        ...
        if (Math.abs(newX - x0) == 1 && newY - y0 == piece.getTeam()) {
            ...
            setTurn(piece.getTeam());
            return new MoveResult(MoveType.NORMAL);
        } else if (Math.abs(newX - x0) == 2 && newY - y0 == piece.getTeam() * 2) {
